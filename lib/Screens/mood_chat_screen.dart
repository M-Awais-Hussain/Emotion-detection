...
class MoodChatScreenState extends State<MoodChatScreen> {
  ...
  Future<String> _getDirectGeminiResponse(String message, String emotion) async {
    // Detect emotion from message if not provided
    String detectedEmotion = _detectEmotionFromMessage(message, emotion);
    
    // Build conversation context
    final conversationHistory = _getFilteredHistory();
    
    // Build prompt with context
    String prompt = _buildPrompt(message, detectedEmotion, conversationHistory);
    ...
    for (int i = 0; i < endpoints.length; i++) {
      String endpoint = endpoints[i];
        try {
        debugPrint('Trying API endpoint ${i + 1}/${endpoints.length}...');
        ...
        if (response.statusCode == 200) {
          ...
          if (data.containsKey('candidates') && 
            ...
            if (candidate.containsKey('content') &&
              ...
              if (text != null && text.toString().trim().isNotEmpty) {
                debugPrint('Successfully got AI response!');
                return text.toString().trim();
              } else {
                debugPrint('Warning: Empty text in response');
              }
            } else {
              debugPrint('Warning: Invalid response structure - no parts found');
            }
          } else {
            debugPrint('Warning: No candidates in response');
          ...
          debugPrint('API Key Error: $errorData');
          throw Exception('Invalid API key. Please check your Gemini API key. Status: ${response.statusCode}');
        } else if (response.statusCode == 429) {
          debugPrint('Rate limit error');
          throw Exception('Rate limit exceeded. Please wait a moment and try again.');
        } else {
          // Try next endpoint for other errors
          final errorBody = response.body;
          debugPrint('API error ${response.statusCode}: ${errorBody.substring(0, errorBody.length > 500 ? 500 : errorBody.length)}');
        ...
        if (e.toString().contains('TimeoutException') ||
          ...
          lastError = Exception('Connection timeout. Please check your internet connection.');
        ...
        if (i == endpoints.length - 1) {
          ...
          // Don't throw, return fallback with emotion-aware response
          break;
        }
        continue;
      }
    ...
    // If all endpoints failed, return emotion-aware fallback
    debugPrint('Returning fallback response for emotion: $detectedEmotion');
    return _getFallbackResponse(detectedEmotion);
  }
  ...
  String _buildPrompt(String message, String emotion, List<Map<String, String>> history) {
    StringBuffer prompt = StringBuffer();
    
    prompt.writeln("You are a compassionate and supportive mental wellness assistant for the Emotion Eye app.");
    prompt.writeln("The user's current detected emotion is: $emotion.");
    prompt.writeln();
    prompt.writeln("Guidelines:");
    prompt.writeln("- Provide empathetic, helpful, and encouraging responses");
    prompt.writeln("- Acknowledge their emotional state");
    prompt.writeln("- Offer practical advice and support");
    prompt.writeln("- Suggest mood improvement activities when appropriate");
    prompt.writeln("- Be warm, understanding, and non-judgmental");
    prompt.writeln("- Keep responses concise (2-3 sentences) and conversational");
    prompt.writeln();
    ...
    if (history.isNotEmpty) {
      ...
      for (var msg in history.take(6)) {
        if (msg['role'] == 'user') {
          prompt.writeln("User: ${msg['text']}");
        } else if (msg['role'] == 'assistant') {
          prompt.writeln("Assistant: ${msg['text']}");
        }
      }
      prompt.writeln();
    }
